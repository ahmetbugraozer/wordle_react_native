...
export default function WordlePage() {
  ...
  const { user, setUser } = useAuth(); // setUser'ı da context'ten alalım
  ...
  useEffect(() => {
    if (!user && gameLogic?.gameFinished) {
      // Giriş yapmamış kullanıcı için günlük limit kontrolü
      AuthService.canPlayToday('').then(canPlay => {
        if (!canPlay) {
          setErrorMessage('Daily limit reached. Please sign in to continue playing.');
        }
      });
    }
  }, [gameLogic?.gameFinished, user]);
  ...
  const handleEnter = async () => {
    ...
    if (gameLogic.gameFinished) {
      if (user) {
        try {
          await AuthService.updateGamesPlayed(user.id);
          // Kullanıcı verilerini güncellemek için context'i yenile
  ...
  return (
    <View style={styles.container}>
      ...
      ) : gameLogic.gameResult ? ( // Kazandıysa modal göster
        <ResultModal
          visible={showResultModal}
          won={gameLogic.gameResult}
          answer={gameLogic.actualWord}
          guesses={gameLogic.wordsEntered.filter(word => word !== '')}
          onClose={() => {
            setShowResultModal(false);
            initializeGame();
          }}
        />
      ) : ( // Kaybettiyse sadece "Game Over" mesajı göster
        <View style={styles.gameOver}>
          <Text style={styles.gameOverText}>Game Over!</Text>
          <View style={styles.loseTextContainer}>
            <Text style={styles.resultText}>The word was </Text>
